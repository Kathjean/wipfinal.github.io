                                            ...
                                            <li>
                                                <form action="<?php echo $fs->_get_admin_page_url('account') ?>" method="POST">
                                                    ...
                                                    <a href="#"
                                                       ...
                                                       ?> <?php echo esc_attr($prices_increase_text) ?> <?php fs_esc_attr_echo_inline('Are you sure you want to proceed?', 'proceed-confirmation', $slug) ?>'))
                                                                               this.parentNode.submit();
                                                                           return false;"><i class="dashicons dashicons-download"></i> <?php echo esc_html($fs->is_only_premium() ? fs_text_inline('Cancel Subscription', 'cancel-subscription', $slug) : $downgrade_text ) ?></a>
                                                </form>
                                            </li>
                                            <li>&nbsp;&bull;&nbsp;</li>
                                        <?php endif ?>
                                    ...
                                    ?>>
                                        ...
                                        <?php elseif ('version' === $p['id'] && $has_paid_plan) : ?>
                                            <?php if ($fs->has_premium_version()) : ?>
                                                <?php if ($is_premium || $fs->can_use_premium_code()) : ?>
                                                    <label class="fs-tag fs-<?php echo $fs->can_use_premium_code() ? 'success' : 'warn' ?>"><?php fs_esc_html_echo_inline('Premium version', 'premium-version', $slug) ?></label>
                                        ...
                                        <td class="fs-right">
                                            ...
                                            <?php if ('version' === $p['id']) : ?>
                                                <?php if ($fs->has_release_on_freemius()) : ?>
                                                    <div class="button-group">
                                                        <a href="<?php echo admin_url('themes.php?page=marmot-theme-setup') ?>" class="button button-primary">
                                                            <?php _ex('Marmot Setup Wizzard', 'license-account', 'hqtheme-extra')?>
                                                        </a>
                                                    </div>
                                                <?php endif ?>
                                            ...
                                            elseif (/* in_array($p['id'], array('site_secret_key', 'site_id', 'site_public_key')) || */
                                                ...
                                                <form action="<?php echo $fs->_get_admin_page_url('account') ?>" method="POST"
                                                      ...
                                                      ?>', '<?php echo $p['value'] ?>');
                                                                          if (null == val || '' === val)
                                                                              return false;
                                                                          jQuery('input[name=fs_<?php echo $p['id'] ?>_<?php echo $fs->get_unique_affix() ?>]').val(val);
                                                                          return true;">
